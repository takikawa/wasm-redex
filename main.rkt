                  ...
                  (seq (get-local 0)
                       ...
                       (if (-> () (i32))
                           ...
                           (seq (get-local 0)
                                ...
                                (sub i64)
                                (call 3)))))))
    (define cl-0  (term {(inst 0) (code ,fac)}))
    (define cl-1  (term {(inst 0) (code ,fac-acc)}))
    (define cl-2  (term {(inst 0) (code ,fib)}))
    (define cl-3  (term {(inst 0) (code ,even)}))
    (define cl-4  (term {(inst 0) (code ,odd)}))
    (define modinst-0
      (term {(func ,cl-0 ,cl-1 ,cl-2 ,cl-3 ,cl-4) (glob)}))
    (define func-store
      (term {(inst ,modinst-0) (tab) (mem)}))
    (define-syntax-rule (func-config e*)
      (term (,func-store () e* 0)))

    (test-wasm-->> (func-config (seq (const i64 5) (call 0)))
                   (func-config (seq (const i64 120))))
    (test-wasm-->> (func-config (seq (const i64 5) (const i64 1) (call 1)))
                   (func-config (seq (const i64 120))))
    (test-wasm-->> (func-config (seq (const i64 5) (call 2)))
                   (func-config (seq (const i64 8))))
    (test-wasm-->> (func-config (seq (const i64 6) (call 3)))
                   (func-config (seq (const i32 44))))
    (test-wasm-->> (func-config (seq (const i64 5) (call 3)))
                   (func-config (seq (const i32 99))))
    (test-wasm-->> (func-config (seq (const i64 4) (call 4)))
                   (func-config (seq (const i32 99))))
    (test-wasm-->> (func-config (seq (const i64 7) (call 4)))
                   (func-config (seq (const i32 44)))))
